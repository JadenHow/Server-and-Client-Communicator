 ...
 * the username, then copy buf to username.  Otherwise, the
...
int read_from(int client_index, struct sockname *users) {
    ...
    if (users[client_index].username == NULL) {
        users[client_index].username = malloc(sizeof(char) * (num_read + 1));
        strcpy(users[client_index].username, buf);
    } else {
        char msg[BUF_SIZE + 1];
        strcpy(msg, users[client_index].username);
        msg[strlen(users[client_index].username) - 1] = '\0';
        strcat(msg, ": ");
        strcat(msg, buf);

        for (int i = 0; i < MAX_CONNECTIONS; i++) {
            int fd1 = users[i].sock_fd;
            if (i != client_index && num_read != 0) {
                write(fd1, msg, strlen(msg));
            }
        }

        if (num_read == 0 || write(fd, msg, strlen(msg)) != strlen(msg)) {
